...
- **Strangler Fig Migration** — the ACL is a prerequisite; once it is in place, you can strangle the legacy system slice by slice without new code absorbing legacy concepts.
...
- **Agentic Coding Agents** — agents generating new services against a legacy system should be given the ACL interface as their target contract, not the raw legacy schema.

## What Next

With an ACL in place at a legacy boundary, use **Strangler Fig Migration** to begin routing functionality to the new system — the ACL provides the translation infrastructure you need to migrate incrementally without a flag day.
