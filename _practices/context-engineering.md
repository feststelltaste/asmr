...
- [Harness Engineering](../harness-engineering/) — context is what the agent *knows*; the harness is what the agent can *do* and how it *verifies* its work.
- [Agentic Coding Workflows](../agentic-coding-workflows/) — the workflow provides the "dynamic" context (current task, error logs), while context engineering provides the "static" context (architectural rules).
- [Human in the Loop](../human-in-the-loop/) — review is the primary feedback loop for updating the engineered context (e.g., "The AI keeps making this mistake, let's add a rule for it").

## What Next

After establishing your first project rules file, implement [Harness Engineering](../harness-engineering/) to build the execution environment where the agent can run and verify the code it writes based on those rules.
