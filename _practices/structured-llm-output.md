...
- **Agentic Coding Workflows** — structured output is the "communication protocol" between different agents in a workflow.
- **AI-driven Technical Debt Detection** — the detection agent produces its report in a structured format for easy import into a backlog.
- **LLM-as-judge Evaluation** — the judge provides its score and reasoning in a structured JSON format for easy analysis.

## What Next

After establishing your first structured output pipeline, implement **LLM-as-judge Evaluation** to automatically score the quality of the structured data your agents are producing.
